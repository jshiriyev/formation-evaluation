def scattered(string,Cmat,Lambda,Q,Hc0p,Jc0p,bnd_r,mur_s,mur_z,nord,Nord,Nb,B2):
    ...
    z_source = linspace(Zstart,Zend,Nsamples).T;
    ...
    for k in range(Nsamples):
        ...
        for i in range(Nord):
            ...
            Hsca_n = 1/mur_z(1,1)*gh_obsrvr.T*Cmat(B2,B2,1)*Q_a(B2,B2,i)*bph;
            if i == 1
                Hsca(k,1) = Hsca(k,1)+Hsca_n;
            else
                Hsca(k,1) = Hsca(k,1)+2*Hsca_n;
    
    return z_obsrvr,Hsca

def sampling(z_dash,qeta,Zglobal,dz,Ne,Nb):
